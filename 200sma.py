...
def format_output(ts, ltp, coi_pcr, change_pts, adx, ma_data):
    ...
    print(f"🕒 {time_str} | {ltp} | {change_pts} {change_emoji} | {adx_str} | MA:{cross_str} | {coi_pcr_k}K{coi_flag} | {ma_str}")
    send_telegram_message(f"🕒{time_str} | {change_pts} {change_emoji} | {adx_str} | MA:{cross_str} | {coi_pcr_k}K{coi_flag}", False)
...
def execute_call_trade(atm, strike):
    ...
    ENTRY_STRIKE    = strike
    send_telegram_message(f"🟢 Entered Call | Strike {strike}")
    print(f"🟢 Entered Call | Strike {strike}")
...
def execute_put_trade(atm, strike):
    ...
    ACTIVE_POSITION = "PUT"
    ENTRY_STRIKE    = strike
    send_telegram_message(f"🔴 Entered Put | Strike {strike}")
    print(f"🔴 Entered Put | Strike {strike}")
...
def monitor_loop():
    ...
    format_output(ts, ltp, coi_pcr, change, LAT_ADX, ma_data)
    ...
    if ACTIVE_POSITION == "CALL":
        ...
        if ma_data["cross_up"] and cltp > cvwap:
            execute_call_trade(atm, round_value)
    ...
    if ACTIVE_POSITION == "PUT":
        ...
        if ma_data["cross_down"] and pltp > pvwap:
            execute_put_trade(atm, round_value)
